...
namespace TextureArrayInspector
{
	public static class TextureArrayTools 
	{
		...
		public static void SetTexture (this Texture2DArray dstArr, Texture2D src, int dstCh, bool apply=true)
		{
			...
			#if UNITY_EDITOR	
			UnityEditor.EditorUtility.CompressTexture(src, dstArr.format, UnityEditor.TextureCompressionQuality.Best);  //de-compress and compress to change the format
			#else
		...
		public static void FillTexture (this Texture2DArray srcArr, Texture2D dst, int srcCh)
		{
		    ...
		    #if UNITY_EDITOR
			UnityEditor.EditorUtility.CompressTexture(tmpTex, dst.format, UnityEditor.TextureCompressionQuality.Best);  //de-compress and compress to change the format
			#else
